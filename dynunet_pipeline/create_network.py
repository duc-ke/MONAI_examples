...
def get_kernels_strides_ke(sizes, spacings):
    strides, kernels = [], []
...
def get_network_ke(properties, patch_size, spacing, deep_supr_num, pretrain_path, checkpoint=None):
    n_class = len(properties["labels"])
    in_channels = len(properties["modality"])
    kernels, strides = get_kernels_strides_ke(patch_size, spacing)
